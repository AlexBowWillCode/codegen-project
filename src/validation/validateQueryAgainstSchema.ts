...
export function validateQueryAgainstSchema(
  ...
  if (query.operation === "mutation") {
    ...
    if (!mutationType) {
      errors.push(
        "Query contains a mutation, but the schema doesn't have a Mutation type"
      );
      return errors;
    }
...
function validateField(
  ...
  if (!schemaField) {
    errors.push(
      `Field '${field.name}' doesn't exist on type '${parentType.name}' at ${currentPath}`
    );
    return;
  }
  ...
  if (field.arguments && field.arguments.length > 0) {
    field.arguments.forEach((arg) => {
      if (!arg.name || !arg.value) {
        errors.push(
          `Invalid argument for field '${field.name}' at ${currentPath}`
        );
      }
    });
  }
  ...
  if (field.subFields && field.subFields.length > 0) {
    ...
    if (!fieldType) {
      errors.push(
        `Cannot find type for field '${field.name}' at ${currentPath}`
      );
      return;
    }
    ...
    if (
      ...
      errors.push(
        `Type '${fieldType.name}' at ${currentPath} cannot have subfields because it is a ${fieldType.kind} type`
      );
      return;
    }
...
function resolveFieldType(
  ...
  if (typeInfo.isList && !typeInfo.typeName) {
    ...
    if (inferredTypeName) {
      const inferredType = schema.types.find(
 ...
 */
function extractTypeInfo(typeRef: GraphQLTypeRef | undefined): {
...
function inferTypeFromFieldName(
  ...
  if (fieldName.endsWith("s")) {
    ...
    const matchingType = schema.types.find(
      (t) => t.name === singularName || t.name === capitalizedName
    );
...
function parseVariableType(typeStr: string): {
  typeName: string;
  isNonNull: boolean;
  isList: boolean;
} {
...
function checkFieldArgumentsForVariable(
  field: GraphQLField2,
  variable: GraphQLVariable,
  schema: GraphQLSchema,
  errors: string[],
  ...
  if (field.subFields) {
    field.subFields.forEach((subField) => {
      checkFieldArgumentsForVariable(
        subField,
        variable,
        schema,
        errors,
