...
import { mapFieldToTypeScript, mapGraphQLTypeStringToTypeScript, } from "./typeMapper.js";
/**
 * Generates TypeScript hooks and types for GraphQL queries
 */
export function generateTypeScript(schema, queries, outputDir) {
    ...
    for (const [fileName, query] of queries.entries()) {
        ...
        // Generate hook file
        generateQueryHook(schema, query, fileName, join(hooksDir, `${hookName}.ts`));
    }
    // Generate types file
 ...
 */
function generateQueryHook(schema, query, sourceFileName, outputPath) {
    ...
    const variablesTypeName = query.variables.length > 0 ? `${queryName}Variables` : "undefined";
 ...
 */
function generateTypesFile(schema, queries, outputPath) {
    // Create directory if it doesn't exist
    const dir = dirname(outputPath);
    if (!existsSync(dir)) {
        mkdirSync(dir, { recursive: true });
 ...
 */
function generateGraphQLClientFile(outputPath) {
    // Create directory if it doesn't exist
    const dir = dirname(outputPath);
    if (!existsSync(dir)) {
        mkdirSync(dir, { recursive: true });
 ...
 */
function generateGraphQLDocument(query) {
 ...
 */
function generateFieldsString(fields, indent) {
    let result = "";
